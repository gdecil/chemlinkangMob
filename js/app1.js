...
//              var date = new Date(value.$date);
  ...
  module.controller('MasterController', function($scope, $data, $http, $rootScope) {
    var $that = this;
//    $scope.items = $data.items;  
    $scope.demoSli = {
        			min1: 80,
...
*/
        ...
        service.getBatchStrid(batch).done(function(strid) {
            ...
            var sql = "<div id='ic'><img id='noresizable' width='400px' height: '200px' src='" + server  + "/render?strid=" + batch[0].strid + "'></div>"
            $('#imgcont').prepend(sql)
        });
    }
